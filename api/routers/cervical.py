"""CERVICAL screening router — exposes mv_cervical_screening.
...
router = APIRouter(prefix="/api/v2/cervical", tags=["CERVICAL Screening"])

_VALID_SCOPES = {"city", "zone", "district", "non_bkk"}


@router.get("/screening")
def cervical_screening(
    scope: str = Query(..., description="zone | city | district | non_bkk"),
    id: Optional[str] = Query(None),
):
    """Aggregated abnormal-screening rate for CERVICAL."""
    ...
    cache_key = f"cervical:screening:{scope}:{scope_id}:s"
    ...
    if scope == "zone":
        rows = execute_query(
            ...
            FROM public.mv_cervical_screening m
        ...
        rows = execute_query(
            """SELECT region_code AS district_code, n_total, n_abnormal,
               ...
               FROM public.mv_cervical_screening_region""",
            None,
        )
    else:  # city
        rows = execute_query(
    ...
    # Per-subdistrict ranking (only for scope='district') from mv_cervical_subdistrict.
    ...
    if scope == "district" and scope_id:
        sub_rows = execute_query(
            ...
            FROM public.mv_cervical_subdistrict
...
# =============================================================================
# /screening/factors/bulk — demographic/lifestyle factor cross-tab (mig 354)
# =============================================================================
...
# labels here.
_CERVICAL_FACTOR_LABELS: dict[str, dict[str, str]] = {
    "sex":       {"th": "เพศ",                    "en": "Sex"},
...
_CERVICAL_GROUP_ORDER: dict[str, list[str]] = {
    "sex":       ["male", "female"],
...
def _shape_cervical_screening_factors(rows: list[dict], scope: str, scope_id: object) -> dict:
    ...
    for fk in _CERVICAL_FACTOR_LABELS:
        groups = by_factor.get(fk, [])
        if not groups:
            continue
        order = {g: i for i, g in enumerate(_CERVICAL_GROUP_ORDER.get(fk, []))}
        ...
        factors_payload.append({
            "factor_key":      fk,
            "factor_label_th": _CERVICAL_FACTOR_LABELS[fk]["th"],
            "factor_label_en": _CERVICAL_FACTOR_LABELS[fk]["en"],
            "groups":          groups,
            "top_group":       top_group,
        })
...
def cervical_screening_factors_bulk():
    """One-shot: city + all zones + all districts of CERVICAL factor cross-tab.

    Cached aggressively under a single static key — payload is identical
    for every reader.
    """
    cache_key = "cervical:screening:factors:bulk"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    rows = execute_query(
        ...
        FROM public.mv_cervical_screening_factors
        """,
        None,
    )
    ...
    payload = {
        "data": {
            "city":      _shape_cervical_screening_factors(city_rows, "city", None),
            "zones":     {zc: _shape_cervical_screening_factors(rs, "zone", zc) for zc, rs in by_zone.items()},
            "districts": {dc: _shape_cervical_screening_factors(rs, "district", dc) for dc, rs in by_dist.items()},
        }
    }
