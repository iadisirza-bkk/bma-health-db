"""XRAY screening router — exposes mv_xray_screening.
...
router = APIRouter(prefix="/api/v2/xray", tags=["XRAY Screening"])
...
def xray_screening(
    ...
    """Aggregated abnormal-screening rate for XRAY."""
    ...
    cache_key = f"xray:screening:{scope}:{scope_id}:s"
    ...
    if scope == "zone":
        rows = execute_query(
            ...
            FROM public.mv_xray_screening m
        ...
        rows = execute_query(
            """SELECT region_code AS district_code, n_total, n_abnormal,
               ...
               FROM public.mv_xray_screening_region""",
            None,
        )
    else:  # city
        rows = execute_query(
    ...
    # Per-subdistrict ranking (only for scope='district') from mv_xray_subdistrict.
    ...
    if scope == "district" and scope_id:
        sub_rows = execute_query(
            ...
            FROM public.mv_xray_subdistrict
...
# =============================================================================
# /screening/factors/bulk — demographic/lifestyle factor cross-tab (mig 353)
# =============================================================================
...
# labels here.
_XRAY_FACTOR_LABELS: dict[str, dict[str, str]] = {
    "sex":       {"th": "เพศ",                    "en": "Sex"},
...
_XRAY_GROUP_ORDER: dict[str, list[str]] = {
    "sex":       ["male", "female"],
...
def _shape_xray_screening_factors(rows: list[dict], scope: str, scope_id: object) -> dict:
    ...
    for fk in _XRAY_FACTOR_LABELS:
        groups = by_factor.get(fk, [])
        if not groups:
            continue
        order = {g: i for i, g in enumerate(_XRAY_GROUP_ORDER.get(fk, []))}
        ...
        factors_payload.append({
            "factor_key":      fk,
            "factor_label_th": _XRAY_FACTOR_LABELS[fk]["th"],
            "factor_label_en": _XRAY_FACTOR_LABELS[fk]["en"],
            "groups":          groups,
            "top_group":       top_group,
        })
...
def xray_screening_factors_bulk():
    """One-shot: city + all zones + all districts of XRAY factor cross-tab.

    Cached aggressively under a single static key — payload is identical
    for every reader.
    """
    cache_key = "xray:screening:factors:bulk"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    rows = execute_query(
        ...
        FROM public.mv_xray_screening_factors
        """,
        None,
    )
    ...
    payload = {
        "data": {
            "city":      _shape_xray_screening_factors(city_rows, "city", None),
            "zones":     {zc: _shape_xray_screening_factors(rs, "zone", zc) for zc, rs in by_zone.items()},
            "districts": {dc: _shape_xray_screening_factors(rs, "district", dc) for dc, rs in by_dist.items()},
        }
    }
