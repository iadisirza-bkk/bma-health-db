"""CKD screening router — exposes mv_ckd_screening.
...
router = APIRouter(prefix="/api/v2/ckd", tags=["CKD Screening"])
...
def ckd_screening(
    ...
    """Aggregated abnormal-screening rate for CKD."""
    ...
    cache_key = f"ckd:screening:{scope}:{scope_id}:s"
    ...
    if scope == "zone":
        rows = execute_query(
            ...
            FROM public.mv_ckd_screening m
        ...
        rows = execute_query(
            """SELECT region_code AS district_code, n_total, n_abnormal,
               ...
               FROM public.mv_ckd_screening_region""",
            None,
        )
    else:  # city
        rows = execute_query(
    ...
    # Per-subdistrict ranking (only for scope='district') from mv_ckd_subdistrict.
    ...
    if scope == "district" and scope_id:
        sub_rows = execute_query(
            ...
            FROM public.mv_ckd_subdistrict
...
# =============================================================================
# /screening/factors/bulk — demographic/lifestyle factor cross-tab (mig 350)
# =============================================================================
...
# labels here.
_CKD_FACTOR_LABELS: dict[str, dict[str, str]] = {
    "sex":       {"th": "เพศ",                    "en": "Sex"},
...
_CKD_GROUP_ORDER: dict[str, list[str]] = {
    "sex":       ["male", "female"],
...
def _shape_ckd_screening_factors(rows: list[dict], scope: str, scope_id: object) -> dict:
    ...
    for fk in _CKD_FACTOR_LABELS:
        groups = by_factor.get(fk, [])
        if not groups:
            continue
        order = {g: i for i, g in enumerate(_CKD_GROUP_ORDER.get(fk, []))}
        ...
        factors_payload.append({
            "factor_key":      fk,
            "factor_label_th": _CKD_FACTOR_LABELS[fk]["th"],
            "factor_label_en": _CKD_FACTOR_LABELS[fk]["en"],
            "groups":          groups,
            "top_group":       top_group,
        })
...
def ckd_screening_factors_bulk():
    """One-shot: city + all zones + all districts of CKD factor cross-tab.

    Cached aggressively under a single static key — payload is identical
    for every reader.
    """
    cache_key = "ckd:screening:factors:bulk"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    rows = execute_query(
        ...
        FROM public.mv_ckd_screening_factors
        """,
        None,
    )
    ...
    payload = {
        "data": {
            "city":      _shape_ckd_screening_factors(city_rows, "city", None),
            "zones":     {zc: _shape_ckd_screening_factors(rs, "zone", zc) for zc, rs in by_zone.items()},
            "districts": {dc: _shape_ckd_screening_factors(rs, "district", dc) for dc, rs in by_dist.items()},
        }
    }
