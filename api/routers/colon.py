"""COLON screening router — exposes mv_colon_screening.
...
router = APIRouter(prefix="/api/v2/colon", tags=["COLON Screening"])
...
def colon_screening(
    ...
    """Aggregated abnormal-screening rate for COLON."""
    ...
    cache_key = f"colon:screening:{scope}:{scope_id}:s"
    ...
    if scope == "zone":
        rows = execute_query(
            ...
            FROM public.mv_colon_screening m
        ...
        rows = execute_query(
            """SELECT region_code AS district_code, n_total, n_abnormal,
               ...
               FROM public.mv_colon_screening_region""",
            None,
        )
    else:  # city
        rows = execute_query(
    ...
    # Per-subdistrict ranking (only for scope='district') from mv_colon_subdistrict.
    ...
    if scope == "district" and scope_id:
        sub_rows = execute_query(
            ...
            FROM public.mv_colon_subdistrict
...
# =============================================================================
# /screening/factors/bulk — demographic/lifestyle factor cross-tab (mig 355)
# =============================================================================
...
# labels here.
_COLON_FACTOR_LABELS: dict[str, dict[str, str]] = {
    "sex":       {"th": "เพศ",                    "en": "Sex"},
...
_COLON_GROUP_ORDER: dict[str, list[str]] = {
    "sex":       ["male", "female"],
...
def _shape_colon_screening_factors(rows: list[dict], scope: str, scope_id: object) -> dict:
    ...
    for fk in _COLON_FACTOR_LABELS:
        groups = by_factor.get(fk, [])
        if not groups:
            continue
        order = {g: i for i, g in enumerate(_COLON_GROUP_ORDER.get(fk, []))}
        ...
        factors_payload.append({
            "factor_key":      fk,
            "factor_label_th": _COLON_FACTOR_LABELS[fk]["th"],
            "factor_label_en": _COLON_FACTOR_LABELS[fk]["en"],
            "groups":          groups,
            "top_group":       top_group,
        })
...
def colon_screening_factors_bulk():
    """One-shot: city + all zones + all districts of COLON factor cross-tab.

    Cached aggressively under a single static key — payload is identical
    for every reader.
    """
    cache_key = "colon:screening:factors:bulk"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    rows = execute_query(
        ...
        FROM public.mv_colon_screening_factors
        """,
        None,
    )
    ...
    payload = {
        "data": {
            "city":      _shape_colon_screening_factors(city_rows, "city", None),
            "zones":     {zc: _shape_colon_screening_factors(rs, "zone", zc) for zc, rs in by_zone.items()},
            "districts": {dc: _shape_colon_screening_factors(rs, "district", dc) for dc, rs in by_dist.items()},
        }
    }
