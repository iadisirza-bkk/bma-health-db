-- =============================================================================
-- Migration 220 — HPT pipeline (mv_hpt_*)
-- =============================================================================
...
--   mv_hpt_classification          BKK district × 4-bit pattern
--   mv_hpt_factors                 BKK zone × 6 factors
--   mv_hpt_factors_district        BKK district × 6 factors
--   mv_hpt_factors_region          non-BKK 4-region × 6 factors
--
-- Pattern semantics:
--   c1 = risk    (risk_hpt on mv_visit_resolved)
--   c2 = diag    (hpt self-reported in vitalsignslf/homehealth)
--   c3 = family  (phpt parent in homehealth)
--   c4 = bp_high  (BP ≥ 140/90)
--
-- has_hpt = c1 OR c2 OR c4 (family/c3 excluded — hereditary risk).
-- =============================================================================

BEGIN;

-- =============================================================================
-- 1. mv_hpt_classification — district × 4-bit pattern (c1c2c3c4)
-- =============================================================================
DROP MATERIALIZED VIEW IF EXISTS public.mv_hpt_classification CASCADE;

CREATE MATERIALIZED VIEW public.mv_hpt_classification AS
WITH
diag_app1 AS (
...
fam_app1 AS (
  SELECT patient_id, bool_or(phpt = 1) AS p_disease
  FROM bma_med.app1_homehealth WHERE patient_id IS NOT NULL GROUP BY patient_id
),
fam_portal AS (
  SELECT patient_id, bool_or(phpt IN ('1','true','TRUE')) AS p_disease
...
CREATE UNIQUE INDEX uq_mv_hpt_classification
  ON public.mv_hpt_classification (district_code, pattern);
CREATE INDEX idx_mv_hpt_classification_dc
  ON public.mv_hpt_classification (district_code);

GRANT SELECT ON public.mv_hpt_classification
  TO bma_med_reader, bma_med_clinician, bma_med_loader,
     bma_etl_writer, bma_dba_admin, bma_api_reader;

COMMENT ON MATERIALIZED VIEW public.mv_hpt_classification IS
  '4-bit HPT classification per BKK district. '
  'pattern = c1c2c3c4 where c1=risk, c2=diag, c3=family, c4=lab. '
  'k-anon: cells with n<5 dropped at MV build.';

REFRESH MATERIALIZED VIEW public.mv_hpt_classification;


-- =============================================================================
-- 2. mv_hpt_factors — zone × 6 factors
-- =============================================================================
DROP MATERIALIZED VIEW IF EXISTS public.mv_hpt_factors CASCADE;

CREATE MATERIALIZED VIEW public.mv_hpt_factors AS
WITH
patient_zone AS (
...
patient_disease AS (
  -- has_hpt = c1 OR c2 OR c4 only (family/c3 excluded — hereditary risk
...
factor_rows AS (
  -- BMI
  SELECT pd.patient_id, pd.zone_code, pd.has_hpt,
...
CREATE UNIQUE INDEX uq_mv_hpt_factors ON public.mv_hpt_factors (zone_code, factor_key, factor_group);
CREATE INDEX idx_mv_hpt_factors_zone ON public.mv_hpt_factors (zone_code);

GRANT SELECT ON public.mv_hpt_factors
  TO bma_med_reader, bma_med_clinician, bma_med_loader,
     bma_etl_writer, bma_dba_admin, bma_api_reader;

COMMENT ON MATERIALIZED VIEW public.mv_hpt_factors IS
  'HPT × risk-factor cross-tab per zone (BKK only). '
  'hpt_n = patients with c1/c2/c4 (family/c3 excluded). k-anon n>=5.';

REFRESH MATERIALIZED VIEW public.mv_hpt_factors;


-- =============================================================================
-- 3. mv_hpt_factors_district — district × 6 factors
-- =============================================================================
DROP MATERIALIZED VIEW IF EXISTS public.mv_hpt_factors_district CASCADE;

CREATE MATERIALIZED VIEW public.mv_hpt_factors_district AS
WITH
patient_dist AS (
...
patient_disease AS (
  ...
  SELECT
    pz.patient_id,
    pz.district_code, pz.zone_code, pz.district_name,
    (COALESCE(r.c1_risk, FALSE)
       OR COALESCE(d.c2_diag, FALSE)
       OR COALESCE(l.bp_high, FALSE)) AS has_hpt,
    COALESCE(f.c3_family, FALSE) AS c3_family
  FROM patient_dist pz
  LEFT JOIN risk_per_patient r ON r.patient_id = pz.patient_id
  LEFT JOIN diag_all          d ON d.patient_id = pz.patient_id
  LEFT JOIN fam_all           f ON f.patient_id = pz.patient_id
  LEFT JOIN labs              l ON l.patient_id = pz.patient_id
...
factor_rows AS (
  -- BMI
  SELECT pd.patient_id, pd.district_code, pd.zone_code, pd.district_name, pd.has_hpt,
         'bmi_cat'::text AS factor_key,
         CASE WHEN b.bmi<23 THEN 'normal'
              WHEN b.bmi<25 THEN 'overweight'
              WHEN b.bmi<30 THEN 'obese'
...
FROM factor_rows
WHERE factor_group IS NOT NULL
GROUP BY district_code, zone_code, district_name, factor_key, factor_group, factor_group_th, factor_group_en
HAVING COUNT(*) >= 5
WITH NO DATA;

CREATE UNIQUE INDEX uq_mv_hpt_factors_district ON public.mv_hpt_factors_district (district_code, factor_key, factor_group);
CREATE INDEX idx_mv_hpt_factors_district_dc ON public.mv_hpt_factors_district (district_code);
CREATE INDEX idx_mv_hpt_factors_district_zc ON public.mv_hpt_factors_district (zone_code);

GRANT SELECT ON public.mv_hpt_factors_district
  TO bma_med_reader, bma_med_clinician, bma_med_loader,
     bma_etl_writer, bma_dba_admin, bma_api_reader;

COMMENT ON MATERIALIZED VIEW public.mv_hpt_factors_district IS
  'HPT × risk-factor cross-tab per district (BKK only). '
  'hpt_n = patients with c1/c2/c4 (family/c3 excluded). k-anon n>=5.';

REFRESH MATERIALIZED VIEW public.mv_hpt_factors_district;


-- =============================================================================
-- 4. mv_hpt_factors_region — non-BKK region × 6 factors (4 regions: N/NE/S/C)
-- =============================================================================
...
patient_disease AS (
  -- has_hpt = c1 OR c2 OR c4 only (family/c3 excluded).
  SELECT
    pr.patient_id,
    pr.region_code,
    (COALESCE(r.c1_risk, FALSE)
...
COMMENT ON MATERIALIZED VIEW public.mv_hpt_factors_region IS
  'HPT × risk-factor cross-tab per non-BKK region. '
  'hpt_n = patients with c1/c2/c4 (family/c3 excluded). k-anon n>=5.';
